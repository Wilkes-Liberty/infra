...
| 🟢 Low | Tailscale Premium activation checklist | Once upgraded: define tag hierarchy, draft ACL policy, enable SSH recording, configure flow log destination. | [docs/TAILSCALE_ACL_DESIGN.md](TAILSCALE_ACL_DESIGN.md) | user — requires Tailscale dashboard action first | After Tailscale Premium is activated |
| 🟢 Low | Tailscale tag taxonomy & ACL design | Specific tags to define (`tag:admin`, `tag:prod-service`, `tag:service-drupal`, etc.) and which users/devices get which. | [docs/TAILSCALE_ACL_DESIGN.md](TAILSCALE_ACL_DESIGN.md) | user + code session | After Tailscale Premium is activated |
