...
_Updated 2026-04-23. Wilkes & Liberty is an active subcontractor on an EPA prime contract (Jeremy holds Tier 4 Public Trust). These items document gaps between current state and full compliance for this active engagement. See `docs/compliance/` for all documents._
...
| 🔴 High | Obtain subcontractor flow-down clause list from EPA prime | FAR 52.244-6 and EPA-specific clauses flowed down by the prime determine exact compliance obligations. Without this list, it's not possible to confirm which NIST/DFARS/EPA-specific requirements apply to this subcontract. | [docs/COMPANY_PROFILE.md](COMPANY_PROFILE.md) | user — request from prime's CO/COR | ASAP; blocks definitive scoping of compliance obligations |
| 🔴 High | Confirm whether EPA data is processed in Jeremy's environment | The subcontract may or may not involve EPA-agency data in Wilkes & Liberty's infrastructure. If it does, EPA data-handling policy, FOIA (5 U.S.C. § 552), and records-retention requirements apply. | [docs/COMPANY_PROFILE.md](COMPANY_PROFILE.md) | user — review with prime | Before any EPA data touches the on-prem or VPS environment |
| 🟡 Medium | Document Tier 4 continuous-evaluation self-reporting obligations | Tier 4 Public Trust positions require self-reporting of specified life events (foreign travel, foreign contacts, financial hardship, cohabitant changes, arrests/charges). The specific triggers and timelines are set by the prime's FSO or the issuing agency. These are not documented anywhere in this repo. | [docs/COMPANY_PROFILE.md](COMPANY_PROFILE.md) | user — consult prime FSO or agency | Before next period of performance renewal or SF-86 re-investigation |
