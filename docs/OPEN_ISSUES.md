...
| 🟡 Medium | Document Tier 4 continuous-evaluation self-reporting obligations | Jeremy's Tier 4 Public Trust position (held as a Gridiron IT W2 employee) requires self-reporting of specified life events (foreign travel, foreign contacts, financial hardship, cohabitant changes, arrests/charges). Self-reporting routes through **Gridiron IT** (clearance sponsor of record) — not through W&L and not directly to the agency or GDIT. Specific triggers and timelines are set by Gridiron IT's FSO. Documented here for Jeremy's awareness as W&L principal. | [docs/COMPANY_PROFILE.md](COMPANY_PROFILE.md) | Jeremy — consult Gridiron IT FSO | Before next period of performance renewal or SF-86 re-investigation |
| 🟡 Medium | OCI / FAR 9.504 analysis — required before W&L bids on EPA-adjacent work | When W&L begins bidding on federal work, conduct an Organizational Conflict of Interest analysis for any bids touching EPA or EPA-adjacent agencies, given the principal's concurrent W2 employment at a firm performing EPA work (Gridiron IT). Spousal financial interests (FAR 9.504) may compound this. Disclose and mitigate as required. | [docs/COMPANY_PROFILE.md](COMPANY_PROFILE.md) | Jeremy — engage counsel when W&L begins bidding | Before submitting any federal proposal touching EPA-adjacent work |
| 🟢 Low | Confirm Gridiron IT outside-employment policy disclosure | Jeremy should verify whether Gridiron IT's employment agreement and/or their FSO requires disclosure of outside business activities (W&L). This is a Jeremy-personal obligation, not a W&L doc deliverable — but documented here for awareness. | [docs/COMPANY_PROFILE.md](COMPANY_PROFILE.md) | Jeremy — check Gridiron IT HR/security policy | ASAP if not already done |
| 🟢 Low | SBA affiliation analysis for future W&L set-aside eligibility | Document that Jeremy's W2 employment at Gridiron IT (a larger federal contractor) does not, under typical 13 CFR 121 readings, create SBA affiliation between W&L and Gridiron IT. Relevant if W&L pursues 8(a), WOSB/EDWOSB, VOSB, or HUBZone programs. Engage an SBA-familiar attorney to confirm before filing. | [docs/COMPANY_PROFILE.md](COMPANY_PROFILE.md) | Jeremy — engage SBA-familiar counsel before set-aside filings | Before W&L files for any set-aside certification |
| 🟡 Medium | Fill PAST_PERFORMANCE.md placeholders | All `[FILL IN by Jeremy]` fields in [docs/PAST_PERFORMANCE.md](PAST_PERFORMANCE.md) must be completed before that document can be used in a live bid: start dates, scope, compliance frameworks, reference contact info, and engagement end reasons for HHS/CMS, USPS OIG, and EPA engagements. | [docs/PAST_PERFORMANCE.md](PAST_PERFORMANCE.md) | Jeremy | Before first federal bid submission |
| 🟡 Medium | Inventory retained CMS compliance artifacts from prior engagement | Jeremy may have retained compliance artifacts from the HHS/CMS subcontract period (SSPs, audit logs, security training certs, ATO documentation). These may be directly referenceable in future federal bids and could save significant rebuild effort. Inventory what exists, note format and location. | [docs/PAST_PERFORMANCE.md](PAST_PERFORMANCE.md), [docs/COMPANY_PROFILE.md](COMPANY_PROFILE.md) | Jeremy | Before first federal bid preparation begins |
| 🟢 Low | Confirm Scope Infotec is available as a past-performance reference | Contact the Scope Infotec point of contact from the HHS/CMS engagement to confirm willingness to serve as a past-performance reference for future W&L federal bids. Record their name, title, email, and phone in PAST_PERFORMANCE.md. | [docs/PAST_PERFORMANCE.md](PAST_PERFORMANCE.md) | Jeremy | Before first federal bid submission |
| 🟢 Low | Confirm ECS Tech / ECS Federal reference availability for Postal OIG engagement | Contact an ECS Tech (now ECS Federal) point of contact or USPS OIG COR from Jeremy's 4-year Postal OIG engagement to confirm willingness to serve as a key-personnel past-performance reference. Record contact info in PAST_PERFORMANCE.md. | [docs/PAST_PERFORMANCE.md](PAST_PERFORMANCE.md) | Jeremy | Before first federal bid submission |
| 🟢 Low | Provide resume for extraction into docs/resume/ | Jeremy's current resume should be attached to this repo as a reference artifact. Extract relevant facts (dates, titles, technologies, certifications, education) into a structured `docs/resume/` directory for use in future RFP key-personnel sections. PII (phone, home address) should not be committed — create a redacted version. | docs/resume/ (to be created) | Jeremy | Before first federal bid preparation begins |
...
| 🟡 Medium | Verify AT&T gateway is in IP Passthrough mode | Jeremy reports it's likely already enabled. Confirm in BGW admin UI: `192.168.1.254` → **Firewall → IP Passthrough** → check that Passthrough is active and assigned to Eero (or will be assigned to the new firewall's MAC). Cross-check: compare Eero's WAN IP to `whatismyipaddress.com` — if they match, single-NAT is confirmed; if Eero shows `192.168.x.x`, double-NAT is in place. See NETWORK_PLAN.md Step 2. | [docs/NETWORK_PLAN.md](NETWORK_PLAN.md) | Jeremy | Before starting network rewrite |
| 🟡 Medium | Decide firewall model before procurement | AT&T Internet 5000 (5 Gbps symmetric) confirmed. Firewalla Gold SE (2.5GbE WAN) would cap at ~2.3 Gbps. Options: Gold Pro (~$900, 10GbE, same UX + Tailscale), UDM Pro Max (~$600, 10GbE, richer platform), or accept Gold SE cap. See NETWORK_PLAN.md §4.1 for full comparison and recommendation. | [docs/NETWORK_PLAN.md](NETWORK_PLAN.md) §4.1 | Jeremy | Before ordering any hardware |
...
| 🟢 Low | Verify Thread border router works when Eero is in bridge mode | Eero Pro 7 Max has a built-in Thread border router used by Eve devices (HomeKit/Thread). Thread operates at 802.15.4 (separate from IP routing) and may work in bridge mode — but Eero does not explicitly document this. Verify Eve devices remain reachable from Home app after Step 6 (bridge mode). HomePods also act as Thread BRs and may take over automatically. See NETWORK_PLAN.md §6 Step 6 sub-check. | [docs/NETWORK_PLAN.md](NETWORK_PLAN.md) §6 | Jeremy | During network rewrite Step 6 |
...
| 2026-04-24 | Confirm Eero model supports bridge mode | Resolved: mesh is Eero Pro 7 Max (Wi-Fi 7, 10GbE WAN, built-in Thread BR). Supports bridge mode — confirmed. NETWORK_PLAN.md updated accordingly. |
| 2026-04-24 | Confirm AT&T fiber plan speed | Resolved: AT&T Internet 5000 (5 Gbps symmetric). Firewall must have 10GbE WAN to avoid bottleneck — see NETWORK_PLAN.md §4.1 for model comparison. |
