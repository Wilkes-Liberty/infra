...
| 17 | **Decide firewall model and procure when ready to start the network rewrite** — AT&T is 5 Gbps; Gold SE (~$450) caps at 2.5 Gbps, Gold Pro (~$900) and UDM Pro Max (~$600) preserve full line rate. See §4.1 comparison. | Before network rewrite | `docs/NETWORK_PLAN.md` §4.1, `docs/OPEN_ISSUES.md` §7 |
| 18 | **Fix BGW IPv4 IP Passthrough MAC mismatch (causing double-NAT now)** — current Passthrough Fixed MAC (`fc:3d:73:91:69:00`) doesn't match Eero Pro 7 Max WAN MAC. BGW admin (`192.168.1.254`) → Connected Devices → find Eero WAN MAC → Firewall → IP Passthrough → update Fixed MAC → power-cycle Eero. Verify: Eero WAN IP is a public IPv4, not `192.168.x.x`. IPv6 is fine. **Will need to repeat after Firewalla insertion** (update MAC to firewall's WAN MAC). | ASAP | `docs/OPEN_ISSUES.md` §7 |
| 19 | **Disable BGW Wi-Fi radios** — BGW is broadcasting `ATT722pjmrv` + `ATT722pjmrv_Guest` alongside Eero mesh. First check BGW Connected Devices for anything on BGW WiFi and migrate to Eero. Then: BGW admin → Home Network → Wi-Fi → disable 2.4 GHz, 5 GHz, 6 GHz, and Guest radios. | ASAP | `docs/OPEN_ISSUES.md` §7 |
