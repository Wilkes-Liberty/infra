...
| 2026-04-23 | Confirmed: Jeremy's federal work is on a W2 (Gridiron IT); W&L has no active federal contracts | Jeremy is W2-employed at Gridiron IT (employment chain: Jeremy → Gridiron IT → GDIT → EPA). W&L is not a party to this contract. W&L compliance posture is aspirational federal-readiness for future W&L bids; principal-credentialing (Tier 4 Public Trust) and spouse-related-party disclosure (SF-86 Section 18 cohabitant scope) are current-relevance only because they apply to Jeremy as an individual cleared person. Flow-down clauses, self-reporting, and the federal-contract device are Gridiron IT / Jeremy-as-individual concerns — not W&L obligations. See COMPANY_PROFILE.md. | Earlier framing treated Jeremy's W2 employment as a W&L subcontract — corrected once employment structure was clarified |
...
| 2026-04-24 | Captured home-office physical security posture and network rewrite plan | Jeremy WFH with Tier 4 Public Trust. Created `docs/compliance/PHYSICAL_SECURITY.md` (PE-family posture doc: trust zone model, camera policy, smart speaker policy, server closet controls, NIST 800-171 PE cross-ref) and `docs/NETWORK_PLAN.md` (Path 2 implementation plan: Firewalla Gold SE in front of Eero in bridge mode, full VLAN design, migration steps, HomeKit test plan, rollback). Path 2 selected over guest-network fallback: Eero guest network does not support mDNS reflection (breaks HomeKit) and provides no per-device ACLs. Network rewrite deferred until after Keycloak Phase A + Tailscale Phase B complete. | Guest network fallback — rejected (mDNS limitation breaks HomeKit; no per-device ACLs). Replace Eero entirely — rejected (expensive, unnecessary; Eero in bridge mode preserves mesh WiFi coverage). |
| 2026-04-23 | W&L corporate past performance captured — HHS/CMS via Scope Infotec | W&L previously served as a lower-tier subcontractor on an HHS/CMS engagement through Scope Infotec for 3+ years, ending 2025. This is real corporate past performance — not a first-time bidder posture. Captured in `docs/PAST_PERFORMANCE.md` §1 and `docs/COMPANY_PROFILE.md` (Past Performance section). Placeholders require Jeremy to fill in dates, scope, and reference contact before any bid submission. | n/a |
| 2026-04-23 | Principal / key-personnel past performance captured — three-agency timeline | Jeremy Michael Cerda has ~10 years of federal contractor experience across three agencies: USPS OIG (~4 years W2 via ECS Tech / ECS Federal), HHS/CMS (W&L principal performer), and EPA (current W2 via Gridiron IT). The Postal OIG and EPA engagements are Jeremy's individual W2 experience; only HHS/CMS is W&L corporate past performance. All three engagements are structured in `docs/PAST_PERFORMANCE.md` §2 for use in RFP key-personnel sections and capability statements. | n/a |
